...
def main():
    ...
    if not run_command("python3 generator.py", "Static Site Generator Rebuild"):
        sys.exit(1)
    print("[20%] Static Site Rebuild completed successfully.")
    ...
    if not status_res.stdout.strip():
        print("[100%] No changes to deploy. Site is up to date!")
        sys.exit(0)
    print("[40%] Git status checked: outstanding changes found.")
        
    # 3. Add files to git
    if not run_command("git add -A", "Git staging"):
        sys.exit(1)
    print("[60%] Files successfully staged for commit.")
    ...
    if not run_command(f'git commit -m "{commit_msg}"', "Git commit"):
        sys.exit(1)
    print("[80%] Git changes committed.")
        
    # 5. Push to GitHub
    if not run_command("git push origin main", "Git push to origin main"):
        sys.exit(1)
    print("[100%] Deployment push complete! Live site will update in a few moments.")
